...
enum PlaceIcon {
    // MARK: - Type-safe PlaceCallout Methods

    static func systemName(for callout: PlaceCallout) -> String {
        switch callout {
    ...
    static func color(for callout: PlaceCallout) -> Color {
        switch callout {
    ...
    // MARK: - Legacy String Methods (for backward compatibility)

    static func systemName(for callout: String) -> String {
        if let placeCallout = PlaceCallout(rawValue: callout.lowercased()) {
            return systemName(for: placeCallout)
        }
    ...
    static func calloutType(from category: MKPointOfInterestCategory?) -> PlaceCallout? {
